...
export function Onboarding({ onComplete, setThemeKey, setLang, T }) {
  ...
  const st = {
    ...
    title: { fontFamily: serif, fontSize: "clamp(42px,10vw,52px)", fontWeight: 300, color: "#e8e8e8", margin: "0 0 40px", letterSpacing: "2px" },
    ...
    langBtn: (active) => ({
      ...
      borderRadius: 99, padding: "12px 32px", color: "#e8e8e8", fontSize: 16, fontFamily: serif, cursor: "pointer", transition: "all 0.3s ease"
    }),
    bodyText: { fontFamily: serif, fontSize: "clamp(24px, 6.5vw, 28px)", color: "rgba(255,255,255,0.85)", lineHeight: 1.5, marginBottom: 40 },
    subText: { fontFamily: sans, fontSize: 13, color: "rgba(255,255,255,0.4)", letterSpacing: "1px", textTransform: "uppercase", marginBottom: 30 },
    ghostBtn: { background: "transparent", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 99, padding: "16px 32px", color: "#e8e8e8", width: "100%", fontSize: 16, fontFamily: serif, cursor: "pointer", transition: "background 0.3s" },
    ...
    skipBtn: {
      ...
      textTransform: "uppercase", cursor: "pointer", textDecoration: "underline", textUnderlineOffset: "4px", transition: "color 0.2s"
    }
  };
  ...
  return (
    <div style={st.page}>
      <div style={st.wrapper}>
        ...
        {screen === 1 && (
          <>
            <p style={st.bodyText}>{hi ? "यह आपकी जगह है। कोई दबाव नहीं। कोई शोर नहीं। बस आपके लिए एक पल।" : "This is your space. No pressure. No noise. Just a moment for you."}</p>
