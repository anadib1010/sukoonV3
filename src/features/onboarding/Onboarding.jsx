...
export function Onboarding({ onComplete, setThemeKey, setLang, T }) {
  ...
  // Pin body background so there's never a white flash
  useEffect(() => {
  ...
  return (
    <div style={{
      ...
      {/* ── KEYFRAMES ── inject orbFloat and breathing animation */}
      <style>{`
        @keyframes orbFloat {
      ...
      <div style={{
        ...
        {s.splash && (
          <div style={{ width: "100%", maxWidth: 340, textAlign: "center" }}>
            ...
            <button className="onboard-btn" onClick={() => go(1)} style={btnBase}>Begin</button>
          </div>
        )}
        ...
        {s.features && (
          <div style={{ width: "100%", maxWidth: 360, padding: "0 4px" }}>
            ...
            <div style={{ marginTop: 28, display: "flex", flexDirection: "column", gap: 12 }}>
              <button className="onboard-btn" onClick={() => go(screen + 1)} style={btnBase}>
                {hi ? "आगे" : "Continue"}
              </button>
              <button onClick={handleComplete} style={skipBtn}>{hi ? "छोड़ें" : "skip"}</button>
            </div>
          </div>
        )}
            ...
            {s.breathe && (
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 36 }}>
                ...
                <div style={{ position: "relative", width: 100, height: 100, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  ...
                  {/* Inner orb — breathes with a different timing for depth */}
                  <div style={{
                    width: 64, height: 64,
                    borderRadius: "50%",
                    background: "radial-gradient(circle at 38% 38%, rgba(255,255,255,0.18), rgba(255,255,255,0.04))",
