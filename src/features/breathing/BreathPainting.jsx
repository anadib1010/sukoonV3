...
export function BreathPainting({ T, lang }) {
      ...
      if (phaseRef.current === "inhale") fillRef.current = Math.min(fillRef.current + .008, 1);
        ...
        if (fillRef.current === 0 && going) {
          ...
          if (cycleRef.current >= TARGET_CYCLES) {
            setDone(true); setGoing(false); phaseRef.current = "idle"; creditSession(3); return;
          }
  ...
  const s = {
    ...
    infoLabel: { fontSize: 13, fontWeight: 600, color: T.accent, marginBottom: 4 },
    infoBody:  { fontSize: 12, color: T.textSoft, lineHeight: 1.5, margin: 0 },
    canvasWrap:{ position: "relative", height: 260, borderRadius: 16, overflow: "hidden" },
    canvas:    (g) => ({ width: "100%", height: "100%", position: "absolute", inset: 0, cursor: g ? "pointer" : "default" }),
    startOverlay: { position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" },
    startBtn:  { background: `${T.accent}22`, border: `1px solid ${T.accent}55`, color: T.accent, fontSize: 14, fontWeight: 500, padding: "12px 32px", borderRadius: 99, pointerEvents: "all", cursor: "pointer" },
    statsRow:  { marginTop: 16, display: "flex", flexDirection: "column", gap: 8 },
    statsTop:  { display: "flex", justifyContent: "space-between", alignItems: "center" },
    statCount: { fontSize: 12, color: T.muted, margin: 0 },
    statPhase: { fontSize: 12, color: T.accent, margin: 0, fontWeight: 500 },
    statHint:  { fontSize: 11, color: T.muted, fontStyle: "italic", margin: 0 },
  };

  if (done) return (
    <div className="fade-in" style={s.container}>
      <span style={s.doneIcon}>🎨</span>
      <h3 style={s.doneTitle}>{isHindi ? "सुंदर।" : "Beautiful."}</h3>
      <p style={s.doneBody}>
  ...
  return (
    <div style={s.container}>
      {!going && (
        <div style={s.infoBox}>
          <p style={s.infoLabel}>{isHindi ? "निर्देश:" : "Instructions:"}</p>
          <p style={s.infoBody}>
            {isHindi
      ...
      <div style={s.canvasWrap} onClick={togglePhase}>
        <canvas ref={canvasRef} style={s.canvas(going)} />
        {!going && (
          <div style={s.startOverlay}>
            <button onClick={e => { e.stopPropagation(); start(); }} style={s.startBtn}>
      ...
      {going && (
        <div style={s.statsRow}>
          <div style={s.statsTop}>
            <p style={s.statCount}>
              {isHindi ? `${cycles} / ${TARGET_CYCLES} सांस` : `${cycles} of ${TARGET_CYCLES} breaths`}
            </p>
            <p style={s.statPhase}>
              {phase === "inhale" ? (isHindi ? "सांस भरें" : "Inhaling...") : (isHindi ? "सांस छोड़ें" : "Exhaling...")}
            </p>
          </div>
          <p style={s.statHint}>{isHindi ? "टैप करें बदलने के लिए" : "Tap the canvas to switch"}</p>
        </div>
      )}
    </div>
  );
