...
export function Practice({ setTab, goBack, T, lang }) {
  ...
  // By default, we keep the first breathing pattern open (Accordion style)
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column", overflow: "hidden", background: T.bg }}>
      ...
      <div className="scroll-area" style={{ flex: 1, overflowY: "auto", padding: "0 18px 100px" }}>
        ...
        {section === "sessions" ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {(filter === "All" ? MEDITATIONS : MEDITATIONS.filter(m => m.cat === filter)).map(m => (
                <div key={m.id}>
                  ...
                  <button onClick={() => toggleMeditation(m)} style={{ width: "100%", background: sel?.id === m.id ? `${m.col}10` : T.surface, border: `1px solid ${sel?.id === m.id ? m.col+"50" : m.col+"22"}`, borderRadius: 18, padding: "16px 18px", display: "flex", alignItems: "center", gap: 14, textAlign: "left" }}>
                  ...
                  {/* ACCORDION CONTENT (INLINE PLAYER) */}
                  {sel?.id === m.id && (running || done) && (
                    <div className="fade-down" style={{ marginTop: 8, padding: "24px 16px", background: "rgba(255,255,255,0.02)", borderRadius: 16, border: `1px solid ${T.borderWarm}`, display: "flex", flexDirection: "column", alignItems: "center" }}>
                      {running ? (
                        <>
                          ...
                          {/* AUDIO PROGRESS BAR */}
                          <div style={{ width: "100%", maxWidth: 220, height: 4, background: T.surfaceAlt, borderRadius: 99, marginBottom: 16, overflow: "hidden" }}>
                            <div style={{ height: "100%", width: `${(1 - (secs / (m.dur * 60))) * 100}%`, background: m.col, transition: "width 1s linear" }} />
                          </div>

                          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                             <button onClick={toggleGuide} style={{ background: `${m.col}15`, border: `1px solid ${m.col}40`, borderRadius: 99, padding: "8px 20px", color: m.col, fontSize: 13 }}>
                               {guidePlaying ? "⏸" : "🎙"} {isHindiLang ? "आवाज़" : "Voice"}
                             </button>
                             <button onClick={() => toggleMeditation(m)} style={{ background: "transparent", border: `1px solid ${T.muted}35`, color: T.muted, fontSize: 13, padding: "8px 20px", borderRadius: 99 }}>
                        ...
                        <>
                          <span style={{ fontSize: 44, marginBottom: 12, display: "block" }}>🌸</span>
                          <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 26, color: T.accentSoft, fontWeight: 400, margin: "0 0 12px" }}>{isHindiLang ? "बहुत अच्छा।" : "Beautiful."}</h2>
                          <button onClick={() => toggleMeditation(m)} style={{ background: `${T.accent}20`, border: `1px solid ${T.accent}45`, color: T.accent, padding: "10px 28px", borderRadius: 99, fontSize: 14 }}>
                            {isHindiLang ? "बंद करें" : "Close"}
                          </button>
                        </>
                      )}
                    </div>
          ...
          ========================================== */
          <>
            {fromRacing && <div style={{ background: `${T.accent}10`, border: `1px solid ${T.accent}28`, borderRadius: 14, padding: "12px 16px", marginBottom: 16 }}><p style={{ fontSize: 13, margin: 0, color: T.textSoft }}>🌀 {isHindiLang ? "दौड़ते विचार? Box Breathing सबसे तेज़ है।" : "Racing thoughts? Box Breathing works fastest."}</p></div>}
            
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
              {BREATHE_PATTERNS.map(p => (
                <div key={p.name}>
                  {/* ACCORDION HEADER BUTTON */}
                  <button onClick={() => toggleBreath(p)} style={{ width: "100%", background: pat?.name === p.name ? `${T.accent}12` : T.surface, border: `1px solid ${pat?.name === p.name ? T.accent + "55" : T.border}`, borderRadius: 16, padding: "14px 18px", display: "flex", alignItems: "center", gap: 12, textAlign: "left" }}>
                    <div style={{ width: 10, height: 10, borderRadius: "50%", background: T.accent, opacity: pat?.name === p.name ? 1 : 0.3 }} />
                    <div style={{ flex: 1 }}>
                      <p style={{ fontWeight: 500, color: T.text, fontSize: 14, margin: "0 0 2px" }}>{p.name}</p>
                      <p style={{ fontSize: 11, color: T.muted, margin: 0 }}>{p.desc}</p>
                    </div>
                  </button>

                  {/* ACCORDION CONTENT (INLINE ANIMATION) */}
                  {pat?.name === p.name && (
                    <div className="fade-down" style={{ marginTop: 8, padding: "24px 16px", background: "rgba(255,255,255,0.02)", border: `1px solid ${T.borderWarm}`, borderRadius: 16, display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
                      <Orb size={150} col={T.accent} pulse={going} label={going ? curPhase.label : null} />
                      
                      {going && (
                        <div style={{ width: "100%", maxWidth: 180, textAlign: "center" }}>
                          ...
                          <p style={{ fontSize: 11, color: T.muted, margin: 0 }}>{isHindiLang ? `चक्र: ${cycles}` : `Cycle: ${cycles}`}</p>
                        </div>
                      )}

                      <button onClick={() => going ? stopBreath(true) : setGoing(true)} style={{ background: going ? `${T.muted}18` : `${T.accent}22`, border: `1px solid ${going ? T.muted + "35" : T.accent + "55"}`, color: going ? T.muted : T.accent, padding: "12px 40px", borderRadius: 99, fontSize: 15, fontWeight: 500 }}>
                        {going ? (isHindiLang ? "रोकें" : "Stop") : (isHindiLang ? "शुरू करें" : "Begin")}
                      </button>
                      
                      {going && <Card T={T} style={{ textAlign: "center", width: "100%", padding: "16px" }}><p style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: "italic", color: T.textSoft, fontSize: 18, margin: 0 }}>{guidance}</p></Card>}
