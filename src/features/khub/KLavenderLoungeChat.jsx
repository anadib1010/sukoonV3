...
const BANNED_FRAGMENTS = [
  ...
  // K-pop specific fan war triggers
  'army trash', 'blink trash', 'army vs', 'blink vs',
];
...
export function KLavenderLoungeChat({ setTab, T, lang }) {
  ...
  const s = {
    ...
    header: {
      ...
      background: `linear-gradient(180deg, ${LAV_COL}18 0%, transparent 100%)`,
      borderBottom: `1px solid ${LAV_COL}28`, textAlign: 'center',
    },
    title: {
      fontFamily: "'Cormorant Garamond', serif", fontSize: '24px',
      fontWeight: 700, color: LAV_COL, letterSpacing: '1px', margin: 0,
    },
    unofficialBadge: {
      display: 'inline-block', marginTop: '6px',
      background: `${LAV_COL}18`, border: `1px solid ${LAV_COL}40`,
      borderRadius: '20px', padding: '3px 10px',
      fontSize: '9px', letterSpacing: '2px', textTransform: 'uppercase',
      color: LAV_COL, opacity: 0.85,
    },
    rulesBtn: {
      marginTop: '8px', background: 'none', border: `1px solid ${LAV_COL}35`,
      borderRadius: '12px', padding: '4px 12px',
      color: LAV_COL, fontSize: '11px', cursor: 'pointer',
    ...
    bubble: (isMe) => ({
      ...
      background: isMe ? LAV_COL : `${T.accent}10`,
      color: isMe ? '#fff' : T.text,
      border: `1px solid ${isMe ? 'transparent' : `${LAV_COL}22`}`,
      fontSize: '14px', lineHeight: '1.6',
      boxShadow: isMe ? `0 4px 14px ${LAV_COL}38` : 'none',
    ...
    inputArea: {
      ...
      borderTop: `1px solid ${LAV_COL}18`,
      display: 'flex', gap: '10px', alignItems: 'center',
    },
    inputField: {
      flex: 1, padding: '13px 20px', borderRadius: '28px',
      background: `${T.accent}06`, border: `1px solid ${LAV_COL}32`,
      color: T.text, outline: 'none', fontSize: '14px',
      fontFamily: "'DM Sans', sans-serif",
    },
    sendBtn: {
      width: '46px', height: '46px', borderRadius: '50%',
      background: LAV_COL, border: 'none', color: '#fff',
      cursor: 'pointer', fontSize: '18px', flexShrink: 0,
      boxShadow: `0 4px 12px ${LAV_COL}42`,
    },
    backBtn: {
      position: 'absolute', left: 16, top: 54,
      background: 'none', border: 'none',
      color: LAV_COL, cursor: 'pointer', fontSize: '20px',
    },
    toast: (type) => ({
    ...
    modal: {
      ...
      border: `1px solid ${LAV_COL}30`,
    },
    modalTitle: {
      fontFamily: "'Cormorant Garamond', serif",
      fontSize: '22px', color: LAV_COL,
      margin: '0 0 4px', fontWeight: 700,
    },
    ruleItem: {
      display: 'flex', gap: '10px', alignItems: 'flex-start',
      padding: '10px 0', borderBottom: `1px solid ${LAV_COL}14`,
      fontSize: '13px', lineHeight: 1.5,
    },
    ruleIcon: { fontSize: '16px', marginTop: '1px', flexShrink: 0 },
    closeBtn: {
      width: '100%', marginTop: '20px', padding: '14px',
      background: LAV_COL, border: 'none', borderRadius: '16px',
  ...
  const rules = hi ? [
    ...
    { icon: '⚔️', hard: true,  text: 'Fandom wars सख्त मना हैं — सभी fandoms यहाँ equal हैं' },
    { icon: '🔞', hard: true,  text: 'NSFW content पूरी तरह banned है — auto-blocked' },
    ...
    { icon: '🎤', hard: false, text: 'Celebrate your bias without attacking others\' biases' },
    { icon: '🔗', hard: false, text: 'Spotify/YouTube links share करें — in-app playback नहीं' },
    { icon: '🚩', hard: false, text: '3 reports = message auto-review में जाएगा' },
    { icon: '💜', hard: false, text: 'Comparison wars नहीं — "मुझे X पसंद है" ✅, "X > Y" ❌' },
  ] : [
    { icon: '🚫', hard: true,  text: 'No hate speech or slurs — zero tolerance, instant mute' },
    { icon: '⚔️', hard: true,  text: 'No fandom wars — all fandoms are welcome here equally' },
    ...
    { icon: '🎤', hard: false, text: 'Celebrate your bias without putting down others\'' },
    { icon: '🔗', hard: false, text: 'Spotify / YouTube links allowed — no in-app playback' },
    { icon: '🚩', hard: false, text: '3 reports from users = message goes to review queue' },
    { icon: '💜', hard: false, text: 'No comparison wars — "I love X" ✅, "X is better than Y" ❌' },
  ];
    ...
    <div style={s.container}>
      ...
      <div style={s.header}>
        ...
        <div style={s.unofficialBadge}>
          {hi ? '⚠️ अनधिकृत फैन कम्युनिटी — किसी K-Pop लेबल से संबद्ध नहीं'
      ...
      <div style={s.chatArea}>
        {messages.length === 0 && (
          <div style={{ textAlign: 'center', opacity: 0.35, marginTop: '40px', fontSize: '13px' }}>
            {hi ? 'पहले message करने वाले बनें! 🪻' : 'Be the first to share! 🪻'}
          </div>
        )}
        {messages.map(m => {
      ...
      <div style={s.inputArea}>
        <input
          style={s.inputField}
          placeholder={hi ? 'कुछ share करें... 🪻' : 'Share your thoughts...'}
          value={input}
        ...
        <div style={s.overlay} onClick={() => setShowRules(false)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize: '11px', opacity: 0.5, marginTop: 0, marginBottom: '16px', letterSpacing: '0.5px' }}>
              {hi
                ? '⚠️ यह JSukoon का एक अनधिकृत K-Pop फैन स्पेस है। HYBE, YG Entertainment, SM Entertainment, JYP Entertainment या किसी भी K-Pop label से कोई आधिकारिक संबंध नहीं है। "BTS", "ARMY", "BLACKPINK", "BLINK" आदि उनके respective companies के trademarks हैं।'
                : '⚠️ This is an unofficial K-Pop fan space on JSukoon. Not affiliated with HYBE, YG Entertainment, SM Entertainment, JYP Entertainment, or any K-pop label. "BTS", "ARMY", "BLACKPINK", "BLINK" and related names are trademarks of their respective companies.'}
            </p>
            {rules.map((r, i) => (
              <div key={i} style={s.ruleItem}>
                <span style={s.ruleIcon}>{r.icon}</span>
                <span>
                  {r.hard && <strong style={{ color: LAV_COL }}>{hi ? '[सख्त] ' : '[HARD RULE] '}</strong>}
                  {r.text}
                </span>
              </div>
            ))}
      ...
      {showReport && (
        <div style={s.overlay} onClick={() => setShowReport(null)}>
          <div style={{ ...s.modal, padding: '24px 24px 36px' }} onClick={e => e.stopPropagation()}>
            ...
            {reportReasons.map(reason => (
              <button
                ...
                style={{
                  ...
                  background: `${LAV_COL}12`, border: `1px solid ${LAV_COL}32`,
            ...
            <button
              style={{ ...s.closeBtn, background: 'transparent', border: `1px solid ${LAV_COL}32`, color: T.text }}
