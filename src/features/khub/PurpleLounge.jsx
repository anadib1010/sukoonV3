...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// 💜 PURPLE LOUNGE — Fan Community Chat
// ⚠️  NOT an official app. Not affiliated with HYBE,
//     Big Hit Music, or BTS in any way.
//     "BTS", "ARMY", "Bangtan" are trademarks of
//     HYBE Corporation. This is a fan-made space only.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
const BANNED_FRAGMENTS = [
  ...
  'blink trash', 'blackpink trash', 'bp flop',
  'army trash', 'army vs', 'bts vs',
];
...
export function PurpleLounge({ setTab, T, lang }) {
  ...
  const s = {
    ...
    header: {
      ...
      background: `linear-gradient(180deg, ${PURPLE_COL}20 0%, transparent 100%)`,
      borderBottom: `1px solid ${PURPLE_COL}30`, textAlign: 'center',
    },
    title: {
      fontFamily: "'Cormorant Garamond', serif", fontSize: '24px',
      fontWeight: 700, color: PURPLE_COL, letterSpacing: '1px', margin: 0,
    },
    unofficialBadge: {
      display: 'inline-block', marginTop: '6px',
      background: `${PURPLE_COL}18`, border: `1px solid ${PURPLE_COL}40`,
      borderRadius: '20px', padding: '3px 10px',
      fontSize: '9px', letterSpacing: '2px', textTransform: 'uppercase',
      color: PURPLE_COL, opacity: 0.85,
    },
    rulesBtn: {
      marginTop: '8px', background: 'none', border: `1px solid ${PURPLE_COL}35`,
      borderRadius: '12px', padding: '4px 12px',
      color: PURPLE_COL, fontSize: '11px', cursor: 'pointer',
    ...
    bubble: (isMe) => ({
      ...
      background: isMe ? PURPLE_COL : `${T.accent}10`,
      color: isMe ? '#fff' : T.text,
      border: `1px solid ${isMe ? 'transparent' : `${PURPLE_COL}25`}`,
      fontSize: '14px', lineHeight: '1.6',
      boxShadow: isMe ? `0 4px 14px ${PURPLE_COL}40` : 'none',
      position: 'relative',
    }),
    reportBtn: {
    ...
    inputArea: {
      ...
      borderTop: `1px solid ${PURPLE_COL}20`,
      display: 'flex', gap: '10px', alignItems: 'center',
    },
    inputField: {
      flex: 1, padding: '13px 20px', borderRadius: '28px',
      background: `${T.accent}06`, border: `1px solid ${PURPLE_COL}32`,
      color: T.text, outline: 'none', fontSize: '14px',
      fontFamily: "'DM Sans', sans-serif",
    },
    sendBtn: {
      width: '46px', height: '46px', borderRadius: '50%',
      background: PURPLE_COL, border: 'none', color: '#fff',
      cursor: 'pointer', fontSize: '18px', flexShrink: 0,
      boxShadow: `0 4px 12px ${PURPLE_COL}45`,
    },
    backBtn: {
      position: 'absolute', left: 16, top: 54,
      background: 'none', border: 'none',
      color: PURPLE_COL, cursor: 'pointer', fontSize: '20px',
    },
    toast: (type) => ({
    ...
    modal: {
      ...
      border: `1px solid ${PURPLE_COL}30`,
    },
    modalTitle: {
      fontFamily: "'Cormorant Garamond', serif",
      fontSize: '22px', color: PURPLE_COL,
      margin: '0 0 4px', fontWeight: 700,
    },
    ruleItem: {
      display: 'flex', gap: '10px', alignItems: 'flex-start',
      padding: '10px 0', borderBottom: `1px solid ${PURPLE_COL}14`,
      fontSize: '13px', lineHeight: 1.5,
    },
    ruleIcon: { fontSize: '16px', marginTop: '1px', flexShrink: 0 },
    closeBtn: {
      width: '100%', marginTop: '20px', padding: '14px',
      background: PURPLE_COL, border: 'none', borderRadius: '16px',
  ...
  const rules = hi ? [
    ...
    { icon: '⚔️', hard: true,  text: 'Fandom wars सख्त मना — दूसरे fandoms पर attack नहीं' },
    ...
    { icon: '💜', hard: false, text: 'अपने bias को celebrate करें — दूसरों को down मत करें' },
    ...
    { icon: '⚔️', hard: true,  text: 'No attacking other fandoms — this is a safe space for all' },
    ...
    { icon: '💜', hard: false, text: 'Celebrate your love — do not put others down' },
    ...
    <div style={s.container}>
      ...
      <div style={s.header}>
        ...
        <div style={s.unofficialBadge}>
          {hi
            ? '⚠️ अनधिकृत फैन कम्युनिटी — HYBE या BTS से संबद्ध नहीं'
            : '⚠️ Unofficial fan community · Not affiliated with HYBE or BTS'}
        </div>
        <br />
      ...
      <div style={s.chatArea}>
        {messages.length === 0 && (
          <div style={{ textAlign: 'center', opacity: 0.35, marginTop: '40px', fontSize: '13px' }}>
            {hi ? 'Purple Lounge में आपका स्वागत है! 💜' : 'Welcome to the Purple Lounge! 💜'}
          </div>
        )}
        {messages.map(m => {
      ...
      <div style={s.inputArea}>
        <input
          style={s.inputField}
          placeholder={hi ? 'Purple Lounge में share करें... 💜' : 'Share your purple energy... 💜'}
          value={input}
        ...
        <div style={s.overlay} onClick={() => setShowRules(false)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ fontSize: '11px', opacity: 0.5, marginTop: 0, marginBottom: '16px', letterSpacing: '0.5px' }}>
              {hi
                ? '⚠️ यह JSukoon का एक अनधिकृत फैन स्पेस है। HYBE Corporation, Big Hit Music या BTS से कोई आधिकारिक संबंध नहीं है। "BTS", "ARMY", "Bangtan Sonyeondan" HYBE के registered trademarks हैं।'
                : '⚠️ This is an unofficial fan space on JSukoon. Not affiliated with HYBE Corporation, Big Hit Music, or BTS. "BTS", "ARMY", and "Bangtan Sonyeondan" are registered trademarks of HYBE Corporation.'}
            </p>
            {rules.map((r, i) => (
              <div key={i} style={s.ruleItem}>
                <span style={s.ruleIcon}>{r.icon}</span>
                <span>
                  {r.hard && <strong style={{ color: PURPLE_COL }}>{hi ? '[सख्त] ' : '[HARD RULE] '}</strong>}
                  {r.text}
                </span>
              </div>
            ))}
      ...
      {showReport && (
        <div style={s.overlay} onClick={() => setShowReport(null)}>
          <div style={{ ...s.modal, padding: '24px 24px 36px' }} onClick={e => e.stopPropagation()}>
            ...
            {reportReasons.map(reason => (
              <button
                ...
                style={{
                  ...
                  background: `${PURPLE_COL}12`, border: `1px solid ${PURPLE_COL}30`,
            ...
            <button
              style={{ ...s.closeBtn, background: 'transparent', border: `1px solid ${PURPLE_COL}32`, color: T.text }}
